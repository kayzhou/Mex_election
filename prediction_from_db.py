...
#    Updated: 2020/09/02 20:00:43 by Zhenkun          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def save_user_snapshot(sess, now):
    ...
    for t in get_tweets(sess, now, now.add(days=1)):
        uid = t.user_id
        if uid not in users:
            users[uid] = [0, 0] # anti-AMLO and AMLO > 0, 1
...
def read_users_from_csv(in_name):
    ...
    for line in tqdm(open(in_name)):
        ...
        # if uid in all_bots:
        #     continue
        users[uid] = [int(v0), int(v1)] # 0 for anti-amlo, 1 for amlo
    print("# of users:", len(users))
    return users
...
if __name__ == "__main__":
    ...
    # start = pendulum.datetime(2020, 1, 15, tz="UTC")
    # end = pendulum.datetime(2020, 6, 1, tz="UTC")
    # predict_from_location(start, end, out_dir="cumFrom01")
